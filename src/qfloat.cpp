...
qfloat QPI = QFLOAT(M_PI);
qfloat Q2PI = QFLOAT(M_PI * 2.f);
qfloat QTWO_PI_OVER_SR = QFLOAT(M_PI * 2.f / 44100.f);
...
qfloat qfloat_sin_interp(qfloat phase) {
	...
	return qmuls(qfloat_sinLUT[i], m1) + qmuls(qfloat_sinLUT[j], m);
}
